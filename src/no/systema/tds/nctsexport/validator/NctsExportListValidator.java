 ...
 *
 */
public class NctsExportListValidator implements Validator {

	/**
	 * 
	 */
	public boolean supports(Class clazz) {
		return SearchFilterNctsExportTopicList.class.isAssignableFrom(clazz); 
	}
	...
	public void validate(Object obj, Errors errors) { 
		SearchFilterNctsExportTopicList record = (SearchFilterNctsExportTopicList)obj;
		...
		if(record!=null){
			...
			if( !this.valueExists(record.getAvd()) && !this.valueExists(record.getSign()) ){
				if( this.valueExists(record.getOpd()) || this.valueExists(record.getLrnNr()) || 
					...
					ValidationUtils.rejectIfEmptyOrWhitespace(errors, "avd", "systema.ncts.export.list.error.null.avdOrSign"); 
				}
			}
