...
public class TdsExportHeaderController {
	...
	public ModelAndView doTdsExportEdit(@ModelAttribute ("record") JsonTdsExportSpecificTopicRecord recordToValidate, BindingResult bindingResult, HttpSession session, HttpServletRequest request){
		...
		if(appUser==null){
			...
			if(action!=null){
				...
				JsonTdsExportSpecificTopicFaktTotalRecord sumFaktTotalRecord = this.tdsExportSumDiffCalculatorMgr.getInvoiceTotalFromInvoices(avd, opd, appUser);
				//COVI-JsonTdsExportSpecificTopicRecord sumTopicRecord = this.getSumOfSpecificFieldsInItemLines(avd, opd, appUser);
				//COVI-JsonTdsExportSpecificTopicFaktTotalRecord sumFaktTotalRecord = this.getInvoiceTotalFromInvoices(avd, opd, appUser);
	 ...
	 * @return
	 */
	@RequestMapping(value="tdsexport_edit_printSkilleArkTopic.do",  method={RequestMethod.GET, RequestMethod.POST })
	public ModelAndView doTdsExportEditPrintSkilleArkTopic(HttpSession session, HttpServletRequest request){
		Map model = new HashMap();
		SystemaWebUser appUser = (SystemaWebUser)session.getAttribute(AppConstants.SYSTEMA_WEB_USER_KEY);
		ModelAndView successView = new ModelAndView("redirect:tdsexport.do?action=doFind&sign=" + appUser.getTdsSign());
		
		String method = "doTdsExportEditPrintSkilleArkTopic [RequestMapping-->tdsexport_edit_printSkilleArkTopic.do]";
		logger.info("Method: " + method);
